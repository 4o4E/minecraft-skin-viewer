...
class MinecraftSkinViewerClient private constructor(
    private val server: SkinViewerServer,
    private val httpClient: HttpClient,
    private val closeHttpClient: Boolean,
) : AutoCloseable {
    constructor(server: SkinViewerServer) : this(server, defaultHttpClient(), closeHttpClient = true)

    constructor(server: SkinViewerServer, httpClient: HttpClient) : this(server, httpClient, closeHttpClient = false)

    constructor(baseUrl: String) : this(SkinViewerServer(baseUrl))

    constructor(baseUrl: String, httpClient: HttpClient) : this(SkinViewerServer(baseUrl), httpClient)

    constructor(baseUrl: Url) : this(SkinViewerServer(baseUrl))

    constructor(baseUrl: Url, httpClient: HttpClient) : this(SkinViewerServer(baseUrl), httpClient)
    ...
    private suspend fun HttpResponse.toException(): SkinViewerHttpException =
        SkinViewerHttpException(status, bodyAsText())
...
class SkinViewerHttpException(
    val status: HttpStatusCode,
    val responseText: String,
) : RuntimeException("Skin viewer request failed: ${status.value} ${status.description}; $responseText")
